...
class TestKnowledgeFunctions:
    ...
    def test_extract_entities_with_types_indic_path(self):
        ...
        with mock.patch(
            "app.qortia.knowledge._get_indic_pipeline", return_value=mock_pipeline
        ), mock.patch(
            "app.qortia.knowledge._INDIC_LABEL_MAP", {"LOC": "location"}
        ):
    ...
    def test_load_spacy_model_sets_global(self):
        ...
        with mock.patch("spacy.load", return_value=mock_nlp), \
    ...
    async def test_ingest_knowledge_dedup_identical_hashes(self):
        from app.qortia.knowledge import ingest_knowledge, KnowledgeIngestRequest
        content = "## Section\n" + ("word " * 60)
        agent = _agent()
        ...
        with mock.patch("app.qortia.knowledge.get_main_pool"), \
    ...
    async def test_summarise_tenant_skips_if_too_recent(self):
        ...
        conn.fetchrow = mock.AsyncMock(return_value={"id": UUID("00000000-0000-0000-0000-000000000001")})
...
class TestReflectFunctions:
    ...
    async def test_call_litellm_reflect_non_200_raises(self):
        ...
        with mock.patch("app.qortia.reflect.get_litellm_client", return_value=mock_client), \
             mock.patch("app.qortia.reflect._build_reflect_prompt", return_value="prompt"), \
             mock.patch("asyncio.timeout", return_value=mock.MagicMock(__aenter__=mock.AsyncMock(), __aexit__=mock.AsyncMock())):
            with pytest.raises(HTTPException) as exc_info:
    ...
    async def test_call_litellm_reflect_invalid_action_raises(self):
        from app.qortia.reflect import _call_litellm_reflect
        from fastapi import HTTPException
        payload = json.dumps({"reflections": [{"action": "DESTROY", "content": "x", "type": "lesson", "importance": 0.9}]})
        mock_resp = mock.MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {
            "usage": {},
    ...
    async def test_embed_single_row_skips_empty_text(self):
        ...
        row = {"text_to_embed": "", "id": UUID("00000000-0000-0000-0000-000000000001"), "tbl": "hindsight_memories", "tenant_id": UUID("00000000-0000-0000-0000-000000000001")}
    ...
    async def test_embed_single_row_handles_failure_increments_attempts(self):
        ...
        with mock.patch("app.qortia.reflect._get_embedding", mock.AsyncMock(side_effect=Exception("litellm down"))), \
    ...
    async def test_reflect_agent_no_recent_returns_early(self):
        ...
        with mock.patch("app.qortia.reflect.get_main_pool"), \
...
class TestRememberFunctions:
    ...
    async def test_fetch_agent_clearance_returns_defaults_on_none(self):
        ...
        with mock.patch("app.qortia.remember.get_main_pool"), \
    ...
    async def test_remember_org_rate_limited_raises(self):
        ...
        ctx, conn = _mock_tx()
        conn.fetchval = mock.AsyncMock(side_effect=[
        ...
        with mock.patch("app.qortia.remember.get_main_pool"), \
             ...
             mock.patch("app.qortia.remember._fetch_agent_clearance", mock.AsyncMock(return_value=(2, "all"))):
...
class TestEntityGraphFunctions:
    ...
    async def test_update_entity_summary_with_mock_litellm(self):
        ...
        with mock.patch("app.qortia.entity_graph.get_litellm_client", return_value=mock_client):
            result = await _update_entity_summary("old summary", "new memory content", "litellm_key")
    ...
    async def test_maybe_update_entity_summary_no_row_returns(self):
        ...
        tenant_id = UUID("00000000-0000-0000-0000-000000000001")
        await _maybe_update_entity_summary(
            conn=conn, tenant_id=tenant_id, agent_id=None,
            entity_text="Ghost", memory_content="", is_org=True,
        )
        conn.execute.assert_not_called()
    ...
    async def test_maybe_update_entity_summary_link_count_not_trigger(self):
        ...
        await _maybe_update_entity_summary(
            conn=conn, tenant_id=tenant_id, agent_id=None,
            entity_text="Bob", memory_content="content", is_org=True,
        )
    ...
    async def test_maybe_update_entity_summary_org_path(self):
        ...
        await _maybe_update_entity_summary(
            conn=conn, tenant_id=tenant_id, agent_id=None,
            entity_text="London", memory_content="London HQ" * 50, is_org=True,
        )
        conn.execute.assert_called()
    ...
    async def test_maybe_dedup_memory_high_similarity_archives(self):
        ...
        pool_mock = mock.MagicMock()
        conn = mock.AsyncMock()
        conn.fetchrow = mock.AsyncMock(return_value={"id": UUID("bbbb0000-0000-0000-0000-000000000001"), "similarity": 0.99})
        conn.execute = mock.AsyncMock()
        pool_mock.acquire.return_value.__aenter__ = mock.AsyncMock(return_value=conn)
        pool_mock.acquire.return_value.__aexit__ = mock.AsyncMock(return_value=False)
    ...
    async def test_populate_graph_batch_empty_rows(self):
        ...
        conn = mock.AsyncMock()
        conn.fetch = mock.AsyncMock(return_value=[])
        conn.transaction = mock.MagicMock(return_value=mock.AsyncMock(
...
class TestRecallHybridPipeline:
    ...
    async def test_recall_type_decision_routes_correctly(self):
        ...
        with mock.patch("app.qortia.recall.get_main_pool"), \
             mock.patch("app.qortia.recall.tenant_transaction", return_value=ctx), \
             mock.patch("app.qortia.recall._fetch_agent_clearance", mock.AsyncMock(return_value=(2, "all"))), \
             mock.patch("app.qortia.common.assert_agent_active", mock.AsyncMock()), \
             mock.patch("app.qortia.recall._recall_decisions", mock.AsyncMock(return_value=[_make_recall_result()])) as mock_dec, \
             mock.patch("app.qortia.recall._safe_record_recall_access", mock.AsyncMock(), create=True):
    ...
    async def test_recall_type_lesson_routes_correctly(self):
        ...
        with mock.patch("app.qortia.recall.get_main_pool"), \
             ...
             mock.patch("app.qortia.recall._fetch_agent_clearance", mock.AsyncMock(return_value=(2, "all"))), \
             mock.patch("app.qortia.common.assert_agent_active", mock.AsyncMock()), \
             mock.patch("app.qortia.recall._recall_lessons", mock.AsyncMock(return_value=[_make_recall_result()])):
            resp = await recall(body, agent)
        assert len(resp.results) == 1
    ...
    async def test_recall_type_short_term_routes_correctly(self):
        ...
        with mock.patch("app.qortia.recall.get_main_pool"), \
             ...
             mock.patch("app.qortia.recall._fetch_agent_clearance", mock.AsyncMock(return_value=(2, "all"))), \
             mock.patch("app.qortia.common.assert_agent_active", mock.AsyncMock()), \
             mock.patch("app.qortia.recall._recall_short_term", mock.AsyncMock(return_value=[_make_recall_result()])):
            resp = await recall(body, agent)
        assert len(resp.results) == 1
    ...
    async def test_recall_hybrid_knowledge_scope(self):
        ...
        with mock.patch("app.qortia.recall.get_main_pool"), \
             ...
             mock.patch("app.qortia.common.assert_agent_active", mock.AsyncMock()), \
             mock.patch("app.qortia.recall._embed_query", mock.AsyncMock(return_value=[0.1] * 1024)), \
             mock.patch("app.qortia.recall._bm25_knowledge", mock.AsyncMock(return_value=[know_result])), \
    ...
    async def test_recall_hybrid_exception_in_search_continues(self):
        ...
        with mock.patch("app.qortia.recall.get_main_pool"), \
             ...
             mock.patch("app.qortia.recall._bm25_private", mock.AsyncMock(side_effect=Exception("bm25 fail"))), \
             mock.patch("app.qortia.knowledge.extract_entities", return_value=[]):
            resp = await recall(body, agent)
    ...
    async def test_recall_with_work_order_id_fires_log_task(self):
        ...
        with mock.patch("app.qortia.recall.get_main_pool"), \
             ...
             mock.patch("app.qortia.recall._fetch_agent_clearance", mock.AsyncMock(return_value=(2, "all"))), \
             mock.patch("app.qortia.common.assert_agent_active", mock.AsyncMock()), \
             mock.patch("app.qortia.recall._recall_episodic", mock.AsyncMock(return_value=[_make_recall_result()])), \
    ...
    async def test_embed_query_cache_hit(self):
        ...
        with mock.patch("app.qortia.recall.get_cached_embedding", return_value=cached_emb), \
    ...
    async def test_embed_query_cache_miss_success(self):
        ...
        with mock.patch("app.qortia.recall.get_cached_embedding", return_value=None), \
...
class TestRecallHelpersMissing:
    def test_dynamic_importance_with_confidence_multiplier(self):
        ...
        base = dynamic_importance(0.5, recall_count=0, last_recalled_at=None, confidence_multiplier=1.0)
        degraded = dynamic_importance(0.5, recall_count=0, last_recalled_at=None, confidence_multiplier=0.5)
        assert degraded < base

    def test_sort_by_importance_returns_sorted_desc(self):
        from app.qortia.recall_helpers import _sort_by_importance
        from app.qortia.models import RecallResult
        low = RecallResult(id="a", type="episodic", scope="private", content="low", importance=0.1, created_at="2024-01-01")
        high = RecallResult(id="b", type="lesson", scope="private", content="high", importance=0.9, created_at="2024-01-01")
...
class TestRecallOutcomeAndEntityGraph:
    ...
    async def test_recall_hybrid_entity_graph_boost_with_entities(self):
        ...
        conn.fetch = mock.AsyncMock(return_value=[
            {"mem_id": UUID("cccc0000-0000-0000-0000-000000000001"), "summary": "Alice is the lead"}
        ])
        ...
        with mock.patch("app.qortia.recall.get_main_pool"), \
             ...
             mock.patch("app.qortia.knowledge.extract_entities", return_value=["Alice", "AuthService"]), \
    ...
    async def test_recall_hybrid_rerank_path(self):
        ...
        with mock.patch("app.qortia.recall.get_main_pool"), \
             ...
             mock.patch("app.qortia.knowledge.extract_entities", return_value=[]), \
             mock.patch("app.qortia.recall_rerank._llm_rerank", mock.AsyncMock(return_value=[r2, r1])):
            resp = await recall(body, agent)
...
class TestReflectBatchAndFullCycle:
    ...
    async def test_process_embedding_batch_with_rows(self):
        ...
        tid = UUID("00000000-0000-0000-0000-000000000001")
        rows = [{"id": UUID("aaaa0000-0000-0000-0000-000000000001"),
                 "tenant_id": tid, "text_to_embed": "embed me",
                 "tbl": "hindsight_memories", "lang": "en",
    ...
    async def test_process_embedding_batch_key_fetch_fails_continues(self):
        ...
        with mock.patch("app.qortia.reflect.get_main_pool", return_value=pool_mock), \
             mock.patch("app.qortia.reflect.get_litellm_key", mock.AsyncMock(side_effect=Exception("vault down"))), \
             mock.patch("app.qortia.reflect._embed_single_row", mock.AsyncMock()) as mock_embed:
    ...
    async def test_reflect_agent_with_recent_and_full_cycle(self):
        ...
        recent_row.__getitem__ = lambda self, k: "The user fixed a memory leak" if k == "content" else None
        ...
        reflections = [{"action": "CREATE", "type": "lesson", "importance": 0.9,
                        "content": "Memory leaks should be fixed promptly"}]

        with mock.patch("app.qortia.reflect.get_main_pool"), \
             mock.patch("app.qortia.reflect.tenant_transaction", return_value=ctx), \
             mock.patch("app.qortia.reflect.get_litellm_key", mock.AsyncMock(return_value="key")), \
             mock.patch("app.qortia.reflect._call_litellm_reflect", mock.AsyncMock(return_value=reflections)), \
             mock.patch("app.qortia.reflect._get_embedding", mock.AsyncMock(return_value=[0.1]*1024)), \
    ...
    async def test_reflect_agent_embed_failure_uses_none(self):
        ...
        reflections = [{"action": "CREATE", "type": "mental_model", "importance": 0.8,
        ...
        with mock.patch("app.qortia.reflect.get_main_pool"), \
             ...
             mock.patch("app.qortia.reflect.get_litellm_key", mock.AsyncMock(return_value="key")), \
             mock.patch("app.qortia.reflect._call_litellm_reflect", mock.AsyncMock(return_value=reflections)), \
             mock.patch("app.qortia.reflect._get_embedding", mock.AsyncMock(side_effect=Exception("embed fail"))), \
             mock.patch("app.qortia.reflect._write_reflections", mock.AsyncMock()) as mock_write, \
             mock.patch("app.qortia.remember._fetch_agent_clearance", mock.AsyncMock(return_value=(2, "all"))), \
             mock.patch("app.qortia.reflect.settings") as mock_settings:
            mock_settings.rerank_model = "claude-haiku-4-5"
    ...
    async def test_write_reflections_unstable_update_path(self):
        ...
        reflections = [{"action": "UPDATE", "id": existing_id,
                        "type": "lesson", "importance": 0.9, "content": "updated content"}]
        ...
        with mock.patch("app.qortia.reflect.get_main_pool"), \
             mock.patch("app.qortia.reflect.tenant_transaction", return_value=ctx), \
             mock.patch("app.qortia.recall_helpers._cosine", return_value=0.5):  # below 0.95 threshold
            await _write_reflections(
                agent_id=aid, tenant_id=tid,
                reflections=reflections, new_embeddings=new_embeddings,
                existing_embeddings=existing_embeddings, existing=existing,
                clearance_order=2, agent_division="all",
            )
        conn.execute.assert_called()
...
class TestKnowledgeIngestionAndSummary:
    ...
    async def test_ingest_knowledge_with_existing_deletes_and_reinserts(self):
        ...
        import hashlib
        from app.qortia.knowledge import split_into_sections, extract_index_fields
        sections = split_into_sections(content)
        ...
        with mock.patch("app.qortia.knowledge.get_main_pool"), \
             ...
             mock.patch("app.qortia.common.assert_agent_active", mock.AsyncMock()), \
             mock.patch("app.qortia.knowledge.assert_agent_active", mock.AsyncMock()), \
             mock.patch("app.qortia.knowledge.extract_index_fields", return_value={
                 "index_summary": "summary", "index_questions": "q", "index_entities": "[]"
             }):
            result = await ingest_knowledge(body, agent)
    ...
    async def test_summarise_tenant_with_enough_handoffs_writes_summary(self):
        ...
        from app.qortia.knowledge import _summarise_tenant
        tid = UUID("00000000-0000-0000-0000-000000000001")
        old_run = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=8)

        handoff_row = mock.MagicMock()
        handoff_row.__getitem__ = lambda s, k: {
            "title": "T", "content": "agent finished the feature",
            "created_at": datetime.datetime(2024, 1, 5), "lang": "en", "agent_name": "Agent X"
        }[k]

        pool_mock = mock.MagicMock()
        conn = mock.AsyncMock()
        conn.fetchrow = mock.AsyncMock(return_value={"id": tid})
        conn.fetch = mock.AsyncMock(return_value=[handoff_row, handoff_row, handoff_row])  # 3 handoffs
        conn.execute = mock.AsyncMock()
        tx = mock.AsyncMock()
        tx.__aenter__ = mock.AsyncMock(return_value=None)
        tx.__aexit__ = mock.AsyncMock(return_value=False)
...
class TestRememberEndpoints:
    ...
    async def test_forget_wrong_agent_raises_403(self):
        ...
        ctx, conn = _mock_tx()
        conn.fetchrow = mock.AsyncMock(side_effect=[
    ...
    async def test_lang_auto_detect_non_english(self):
        ...
        body = RememberRequest(memories=[
            MemoryItem(type="episodic", content="Le développeur préfère utiliser Python pour les scripts.")
        ])
        ...
        with mock.patch("app.qortia.remember.get_main_pool"), \
             ...
             mock.patch("app.qortia.entity_graph._maybe_dedup_memory", mock.AsyncMock(return_value=None)):
